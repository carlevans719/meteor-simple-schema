...
var complexMultiType = new SimpleSchema({
	name: {
		type: String
	},
	validUntil: {
...
if (Meteor.isClient) {
 ...
 Tinytest.add("SimpleSchema - Complex Multiple Types", function(test) {
	var sc = validate(complexMultiType, {
		name: "Foo Bar",
		validUntil: [new Date()]
	});
	test.equal(sc.invalidKeys(), []);
	...
	sc = validate(complexMultiType, {
		name: "Foo Bar",
		validUntil: [new Date(), new Date()]
	});
	test.equal(sc.invalidKeys(), []);

	sc = validate(complexMultiType, {
		name: "Foo Bar",
		validUntil: [new Date(), false]
	});
	test.length(sc.invalidKeys(), 1);
